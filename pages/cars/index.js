...
function CarCategoryPage() {
  ...
  return (
    <>
      ...
      <div className={`grid grid-cols-5 gap-5 ${locale === "fa" && "font-iransans"}`}>
        <section className="px-3 col-span-5 lg:col-span-1">
          <div className="bg-white dark:bg-[#243137] dark:text-white shadow-sm rounded-md p-8 container mx-auto xl:max-w-screen-2xl mt-5 lg:mt-0">
            <>
              <p className="mb-3 text-gray-400 dark:text-white">{locale==="en" ? "TYPE" : "مدل"}</p>
              {categories?.map((cat) => {
                return (
                  <SelectBox
                    key={cat.id}
                    title={cat.category}
                    title_fa={cat.category_fa}
            ...
            <p className="mb-12 mt-10 text-gray-400 dark:text-white">
              {locale==="en" ? "Select your Price Range :" : ": بازه قیمتی را انتخاب کنید"}
            </p>
            ...
            />
            <p className="text-sm">{locale==="en" ? "Start Price :" : "شروع قیمت : $"} {priceValue[0]} {locale==="en"&& "$"}</p>
            <p className="text-sm my-2">{locale==="en" ? "Final Price :" : "قیمت نهایی : $"} {priceValue[1]} {locale==="en"&& "$"}</p>
            <button
              onClick={() => priceFilter()}
            ...
            >
              {locale==="en" ? "RESET" : "پیشفرض"}
            </button>
          </div>
        </section>

        <div className="lg:col-span-4 md:col-span-5 col-span-5 px-1 md:px-0">
